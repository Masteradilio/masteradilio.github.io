...
TARGET_BRANCH = 'career-assistant-live-llm-only-20260822'
...
# Current question controls retrieval. History is used only for genuinely vague follow-ups.
s = text.find('function selectRepos(question, history) {')
e = text.find('function sanitizeGithubPath(', s)
if s < 0 or e < 0:
    raise SystemExit('selectRepos block not found')
new_select = r'''function selectRepos(question, history) {
  ...
  const vagueFollowUp = q.split(/\s+/).filter(Boolean).length <= 7 || /^(why|how|what about|and|e|por que|porque|como|e quanto)/.test(q);
  if (vagueFollowUp) {
...
'''
text = text[:s] + new_select + text[e:]

# Remove all question-specific substantive fallback answers.
s = text.find('function localFallback(')
e = text.find('async function repairAnswer(', s)
if s < 0 or e < 0:
    raise SystemExit('localFallback block not found')
...
'''
text = text[:s] + unavailable + text[e:]
...
'''
if old not in text:
    raise SystemExit('api-key fallback block not found')
text = text.replace(old, new)
text = text.replace("reply = repaired?.reply || localFallback(question, language, plan, repos);", "reply = repaired?.reply || '';")
...
if old_end not in text:
    raise SystemExit('terminal fallback block not found')
text = text.replace(old_end, new_end)

# Failed repair in a tool-call path must try the next live model.
text = text.replace(
...
# Mark successful substantive answers explicitly as live LLM + RAG.
text = text.replace(
...
# Catch internal-rule leakage observed in live testing.
text = text.replace(
    "'scratch work','chain of thought'];",
    "'scratch work','chain of thought','non-negotiable rule','rule states','final-answer contract','evidence rules','available evidence blocks'];"
)

# Catch employer attribution errors and DataSUS synthetic overclaims.
needle = '''function needsRepair(text, plan) {
...
'''
replacement = '''function containsProfessionalAttributionError(text) {
...
if 'localFallback(' in text or 'grounded-local-fallback' in text:
    raise SystemExit('prewritten substantive fallback remains')

worker.write_text(text, encoding='utf-8')

# Update evaluation contract.
